...
lv_obj_t *create_nav_panel(lv_obj_t *parent, nav_callback_t home_cb, nav_callback_t settings_cb, nav_callback_t back_cb) {  
    ...
    lv_obj_set_size(nav_panel, lv_pct(100), 90);  // Ancho 100%, alto 90px
    ...
    lv_obj_set_style_pad_all(nav_panel, 0, LV_PART_MAIN);  // Quitar padding interno
    lv_obj_set_style_clip_corner(nav_panel, true, LV_PART_MAIN);  // Recortar contenido fuera de los límites
    ...
    lv_obj_set_width(logo_img, 70);  // Ancho máximo del logo
    lv_obj_set_height(logo_img, 70); // Altura máxima del logo
    ...
    lv_label_set_text(title, "MCV - Siembra y masa");
    ...
    lv_obj_set_size(btn_home, 110, 60);
    ////lv_obj_align(btn_back, LV_ALIGN_RIGHT_MID, -10, 0);    // Alineado cerca del centro
	lv_obj_align(btn_home, LV_ALIGN_RIGHT_MID, -10, 0);    // Alineado cerca del centro
    ...
    lv_obj_set_size(btn_settings, 110, 60);
    ...
    lv_obj_center(label_settings); // Centrar la etiqueta dentro del botón
    ...
    lv_obj_set_size(btn_back, 110, 60);
    lv_obj_align(btn_home, LV_ALIGN_CENTER, -120, 0); // Alineado a la derecha
    lv_obj_t *label_back = lv_label_create(btn_back);
    lv_label_set_text(label_back, "Atras");
    lv_obj_add_style(label_back, &style_button_label, 0); // Aplicar el estilo
    lv_obj_center(label_back); // Centrar la etiqueta dentro del botón
